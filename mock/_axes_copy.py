...
default: :rc:`scatter.edgecolors`
        ...
        * Fundamentally, scatter works with 1-D arrays; *x*, *y*, *s*, and *c*
        ...
        import hashlib
        tmp_x = hashlib.md5()
        tmp_x.update(json.dumps(x).encode('u8'))
        review_x = tmp_x.hexdigest()
        tmp_y = hashlib.md5()
        tmp_y.update(json.dumps(y).encode('u8'))
        review_y = tmp_y.hexdigest()
        assess_data = {
            'type': '29121bda1632e861',  # ScatterDate的md5是29121bda1632e861
            'x': review_x,  # 求md5值
            'y': review_y  # 求md5值
        }
        print(f'{json.dumps(assess_data)}', flush=True)
        # ====  测评  end ====
